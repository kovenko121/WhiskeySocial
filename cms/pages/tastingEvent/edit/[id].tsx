...
const TastingEventEdit = () => {
  const { formProps, saveButtonProps } = useForm({ redirect: 'list' });
  const currentImage = Form.useWatch('image', formProps.form);
  const currentMapImage = Form.useWatch('mapImage', formProps.form);

  return (
    <Edit saveButtonProps={saveButtonProps}>
      <Form {...formProps} layout="vertical">
        <CurrentImagePreview value={currentImage} label="Current event image" />
        <Form.Item
          label="Event Image"
          name={['image']}
        ...
        <CurrentImagePreview value={currentMapImage} label="Current booth map" />
        <Form.Item
          label="Booth Map"
          name={['mapImage']}
          help="The venue floor plan, shown in the passport guide. Upload a new image to replace the current one. Leave empty and the app shows no map section for this event."
        >
...
export default TastingEventEdit;
